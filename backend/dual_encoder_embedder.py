...
class DualEncoderEmbedder:
    ...
    def _create_fusion_network(self):
        ...
        return nn.Sequential(
            nn.Linear(1024 + 768, 1024),  # CLIP-L (1024) + ViT-MAE (768) = 1792
            nn.LayerNorm(1024),
            nn.GELU(),
            nn.Dropout(0.1),
    ...
    def extract_clip_features(self, image):
        ...
        with torch.no_grad():
            ...
            final_features = vision_outputs.last_hidden_state[:, 0]  # CLS token [1024]
            mid_features = vision_outputs.hidden_states[-3][:, 0]    # Earlier layer [1024]
