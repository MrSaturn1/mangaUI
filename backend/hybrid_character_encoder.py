#!/usr/bin/env python3
"""
Hybrid character encoder inspired by DiffSensei's approach.
...
class HybridCharacterEncoder:
    """
    Hybrid character encoder combining CLIP and Magi v2 approaches.
    Based on DiffSensei's multi-encoder architecture.
    """
    ...
    def detect_character_bbox(self, image):
        """
        Detect character bounding box using simple contour detection.
        This is a simplified version - DiffSensei uses more sophisticated face/person detection.
        ...
        try:
            ...
            # Find the largest contour (assume it's the character)
            ...
            print(f"Error detecting character bbox: {e}")
            return None
    ...
    def crop_character(self, image, bbox=None):
        ...
        """
        if bbox is None:
            bbox = self.detect_character_bbox(image)
            
        if bbox is None:
            # If no bbox detected, return center crop
            w, h = image.size
            crop_size = min(w, h)
            left = (w - crop_size) // 2
            top = (h - crop_size) // 2
    ...
    def extract_clip_embedding(self, image, character_name):
        ...
        try:
            ...
            # Process with CLIP
            inputs = self.clip_processor(images=cropped_image, return_tensors="pt")
    ...
    def extract_magi_v2_embedding(self, image, character_name):
        ...
        try:
            ...
            pixel_values = transform(cropped_image).unsqueeze(0).to(self.device)
...
def main():
    ...
    encoder = HybridCharacterEncoder(
